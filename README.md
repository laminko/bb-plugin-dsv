...
![DSV Table showing a sample CSV file with 100,000 rows and 20 columns, and the Viewer pane with the record of row 1 as a read-only form](screenshot.png)

The image shows a synthetic sample file: 100,000 rows × 20 columns, 15 MB. The names, cities, and `@example.com` emails are not real. The Viewer pane on the right shows row 1 on the Record tab, as a read-only form.
...
- **Viewer:** a pane on the right of the grid or below it. A switch in the pane moves it, and the close button next to it closes the pane. Drag the edge of the pane to resize it, and double-click the edge for the default size. The plugin remembers the side and the size. The Cell tab shows the whole value of the active cell, with its line breaks and character count. The Record tab shows the active row as a read-only form: a label and a boxed value for each column. You can select and copy a value there. A long value scrolls inside its box. Its First, Previous, Next, and Last buttons move through the rows you see.
...
- Sort, column widths, hidden columns, and the Viewer pane reset when the tab closes. Only the side and the size of the pane are kept.
